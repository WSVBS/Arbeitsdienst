...
try {
    switch ($getForm) {
        case 'savedatefilter':
            ...
            $url = SecurityUtils::encodeUrl($gNavigation->getUrl(), array('show_option' => 'main',
        ...
        case 'save':
            $datavalue['org_id'] = $gCurrentOrgId; //$orgId;
        ...
        case 'savecat':
            ...
            //erst prüfen, ob der Eintrag schon vorhanden ist

            $categorie = $datavalue['cat_name_intern'];
            $sqlverify = 'SELECT cat_type, cat_name_intern 
                          FROM ' . TBL_CATEGORIES . '
                          WHERE  cat_type = \'ADC\'
                          AND    cat_name_intern = ? -- \'$categorie\'';

            //$vresult = array();
            $vresult = $gDb->queryPrepared($sqlverify, array($categorie));
            $Anzahl = $vresult->rowCount();
            
            //Eintrag wird nur gespeichert, wenn er noch nicht vorhanden ist
            if ($Anzahl === 0)
            {        
